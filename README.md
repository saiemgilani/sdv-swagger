...
| Spec | API | Source |
|---|---|---|
| `nfl_api_openapi.yaml` | NFL.com "Shield" data API (`api.nfl.com`) — 33 paths: anonymous device-token mint, `/football/v2/*` (incl. live team + player statistics per game, `stats/historical/*`, prospects/draft, playoffs), `/experience/v1,v2/gamedetails*` (by game, slug, and team-season), `/live/v1` listings. All verified with the anonymous token during a live game (2026-09-15). | sdv-internal-refs `nfl/` + `nfl/captures/shield/` |
| `nflpro_public_openapi.yaml` | **NFL Pro public tier** (`pro.nfl.com/api/*`) — 12 paths that need **no auth** (no token, no cookies): schedules (current/weeks/games/game/standings/week odds), live scores, team directory + rosters, per-game box score and NGS gamecenter. Teams keyed by 4-digit NGS team id, games by NGS `gameId`. | sdv-internal-refs `nfl/nflpro/` + `captures/public/2026-09-14/` |
| `nflpro_secured_openapi.yaml` | **NFL Pro** Next Gen Stats (`pro.nfl.com/api/secured/*`) — the full per-play NGS surface, behind an NFL+ Premium paywall (`nextgenstats.nfl.com` still serves leaderboards and highlight-play tracking anonymously, but denies per-game lookups). 18 paths (passing/rushing/receiving/defense-overview/defense-nearest/team-offense/team-defense/fantasy × season/week/game scopes, plus per-game play list and NGS win probability). User-bound bearer auth (client-credentials tokens are NOT sufficient — see the spec's `info.description`). | sdv-internal-refs `nfl/nflpro/` — live captures against a subscribed account |
