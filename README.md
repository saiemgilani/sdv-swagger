...
| Spec | API | Source |
|---|---|---|
| `nfl_api_openapi.yaml` | NFL.com "Shield" data API (`api.nfl.com`) — 18 paths: anonymous device-token mint, `/football/v2/*` (incl. live team + player statistics per game), `/experience/v1/gamedetails`, `/experience/v2/gamedetails`, `/experience/v1/gamedetailsbyslug`. All verified with the anonymous token during a live game (2026-09-15). | sdv-internal-refs `nfl/` + `nfl/captures/shield/` |
