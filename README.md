...
| `nflpro_secured_openapi.yaml` | **NFL Pro** Next Gen Stats (`pro.nfl.com/api/secured/*`) — the full per-play NGS surface, behind an NFL+ Premium paywall (`nextgenstats.nfl.com` still serves leaderboards and highlight-play tracking anonymously, but denies per-game lookups). 16 paths (passing/rushing/receiving/defense-overview/defense-nearest/team-offense/team-defense/fantasy × season/week/game scopes), 1,036 field schemas. User-bound bearer auth (client-credentials tokens are NOT sufficient — see the spec's `info.description`). | sdv-internal-refs `nfl/nflpro/` — live captures against a subscribed account |
